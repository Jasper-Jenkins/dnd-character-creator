...
const Selection = (props) => {
    ...
    switch (props.character.navigation) {
        ...
        case 'Ability-Scores':
            let abilityScores = props.character.abilityScores.results.map((abilityScore, index) => {
                return (<button onClick={() => props.character.getScore(abilityScore.index)} className='btn btn-primary col-2' key={index}>{abilityScore.name}</button>)
            });
            return (
                <div className='col-12 text-center selection'>
                    ...
                    <div className='row'>
                        <div className='col-12'>
                            <AbilityScoresForm handleSubmit={props.character.handleSubmit} />
                        </div>
                    </div>
    ...
    //switch (props.character.category) {
    //    case 'races':
